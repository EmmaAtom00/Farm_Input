...
interface SpendingItem {
  ...
  amount: number;
  date: string;
  category: 'seeds' | 'fertilizer' | 'pesticide' | 'equipment' | 'labor' | 'other';
}

const MySpending = () => {
  const [spendingData, setSpendingData] = useState<{ period: string, total: number, breakdown: any[] } | null>(null)
  ...
  const fetchSpendingData = async () => {
    try {
      ...
      const trendsData = await getSpendingTrends();
      ...
      if (summaryData) {
        ...
        if (summaryData.breakdown) {
          const transformedItems = summaryData.items.map((item: any) => ({
            id: item.id,
            type: item.category,
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.container}>
        <Text style={{ textAlign: 'center', marginTop: 20 }}>Loading spending data...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          renderItem={({ item: [category, amount] }) => (
            <View style={styles.categoryCard}>
              ...
              <Text style={styles.categoryPercent}>
                {totalSpending > 0 ? ((amount / totalSpending) * 100).toFixed(0) : 0}%
              </Text>
            </View>
          )}
      ...
      <View style={styles.listHeader}>
        <Text style={styles.listTitle}>{spendingData?.period} Recent Spending</Text>
      </View>
